...
from app.services.stop_validation import validate_stop_references
    ...
    for s in ordered:
        out.append(
            JourneyStop(
                ...
                naptan_access_node_id=s.naptan_access_node_id,
                label=s.label,
...
def _journey_load_options():
    return (
        ...
        selectinload(Journey.stops).selectinload(JourneyStop.naptan_access_node),
    )


def create_journey(db: Session, data: JourneyCreate) -> Journey:
    validate_stop_references(db, data.stops)
    j = Journey(
        title=data.title,
