...
- `dark_skies/uk_dark_sky_places_expanded.csv`：**扩充版**（数十条），覆盖英国及苏格兰/威尔士/北爱常见 **IDA 暗夜场所**、国家公园观星代表点、教育/对比用高灯害锚点；坐标多为**近似中心点**，报告请交叉引用 [IDA Finder](https://www.darksky.org/our-work/conservation/idsp/finder/) 与各地公园官网，并说明作业演示性质。
- `dark_skies/national_parks_england_ogl_centroids.csv`：由 **Natural England** 发布的 **National Parks (England)** GeoJSON（**EPSG:27700**）计算**多边形质心**，再经 **pyproj** 转为 **WGS84** 生成；共 10 个英格兰国家公园，**OGL** 许可。若你手动下载了 `National_Parks_England.geojson`，可在 `code` 目录执行：  
  `python -m scripts.national_parks_geojson_to_csv --geojson "<你的 .geojson 路径>"`  
  重新生成该 CSV（详见脚本文件头说明）。
...
- `film_literary/filming_locations_uk_expanded.csv`：**扩充版**（数十条，以英国及北爱为主），汇总常见**影视/剧集公开取景地**与粗略坐标，供叙事锚点 API 使用；**非片方官方全库**，报告请写明「二次整理 + 维基/旅游局交叉验证」及检索日期。
...
- 本项目已在 `code/scripts/import_local_data.py` 提供示例：将 `film_literary/`、`heritage/`、`dark_skies/` 下 **sample + expanded** CSV **依次导入** SQLite（与 `literary_locations`、`dark_sky_sites` 表对应；重复 `title+latitude` / `name+latitude` 会跳过）。在 `code` 目录执行：`python -m scripts.import_local_data`（详见 `code/README.md`）。
...
## 8. 自动下载到 `data/downloads/`（与本节链接对应）

`DATASETS.md` 中的链接**多数是门户网站**，并非单个「点一下即得到一个 .csv」；全量 NaPTAN 等体积极大，不适合直接塞进 Git。

已提供脚本与离线包说明：

- **脚本**：`data/scripts/download_from_datasets_md.py`（在项目根目录执行：`python data/scripts/download_from_datasets_md.py`）
- **输出目录**：`data/downloads/`（见同目录 **`README.md`** 与 **`DOWNLOAD_LOG.md`**）
- **当前已尽量拉取**：DfT **NaPTAN CSV 前 2MB 样本**、**NPTG localities 完整 CSV**、CKAN **naptan** 元数据 JSON、若干**网页 HTML 存档**（检索页、Kaggle/Google Dataset Search、ORR 首页、Historic England Hub、Natural England 等）。
- **常需浏览器手动**：UNESCO（Cloudflare）、IDA、部分 gov.uk 专题页改版 404、ORR **车站使用量 Excel**、VisitBritain 等——以浏览器打开本节原始链接下载或另存为即可。
