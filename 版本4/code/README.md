...
- **与代码的关系**：应用启动时 `app/seed.py` 会先写入一批**内置种子**；`film_literary/` 与 `dark_skies/` 下另有 **`filming_locations_uk_expanded.csv`、`uk_dark_sky_places_expanded.csv`、`national_parks_england_ogl_centroids.csv`**（与 `*_sample.csv` 一起由脚本**依次导入**）。英格兰国家公园边界更新后，可用 `python -m scripts.national_parks_geojson_to_csv --geojson <路径>` 再生成质心 CSV。你额外下载或维护的 CSV 也可按同列名追加后**合并导入**（已存在的 `title+latitude` / `name+latitude` 会跳过，避免重复）。
