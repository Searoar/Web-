...
def main() -> None:
    ...
    film = root / "film_literary" / "filming_locations_uk_sample.csv"
    heritage = root / "heritage" / "nhle_world_heritage_sites_uk_sample.csv"
    skies = root / "dark_skies" / "uk_dark_sky_places_sample.csv"

    Base.metadata.create_all(bind=engine)
    db = SessionLocal()
    try:
        seed_reference_data(db)
        af, sf, tf = import_film_literary(db, film)
        bh, sh, th = import_heritage(db, heritage)
        ck, sk, tk = import_dark_skies(db, skies)
        db.commit()
        print("导入结果（新增 / 跳过=与库中已有重复 / CSV 有效行）：")
        print(f"  film_literary : +{af} 新增, 跳过 {sf} 条, CSV 共 {tf} 行")
        print(f"  heritage      : +{bh} 新增, 跳过 {sh} 条, CSV 共 {th} 行")
        print(f"  dark_skies    : +{ck} 新增, 跳过 {sk} 条, CSV 共 {tk} 行")
